...
// 'use strict' aktiverar strikt läge i JavaScript vilket:
// - Kräver att alla variabler måste deklareras innan användning
// - Förhindrar användning av reserverade ord som variabelnamn
// - Förhindrar tilldelning till skrivskyddade egenskaper
// - Gör det lättare att upptäcka vanliga programmeringsfel
// Detta är särskilt viktigt i större applikationer som denna städavtalshantering
"use strict";

// ================== DATAANALYS OCH DATASTRUKTURER ==================
...
class FormHandler {
    ...
    // Initialisera dropdown-listor med data från analyser
    initializeDropdowns() {
    ...
    setupEventListeners() {
        // Avtalsnamnsfält
        document.getElementById('contractName').addEventListener('input', e => {
        ...
        ['8036', '8035', '8032'].forEach(activity => {
            ...
            // Ansvarskod för aktivitet
            document.getElementById(`ansvar${activity}`).addEventListener('change', e => {
                this.handleActivityAnsvarChange(activity, e.target.value);
            });
        });
    ...
    // Sätt upp autocomplete för avtalsnamnsfältet
    setupAutoComplete() {
        const searchInput = document.getElementById('contractName');
        const suggestionsList = document.createElement('ul');
        suggestionsList.className = 'suggestions-list';
        searchInput.parentNode.appendChild(suggestionsList);

        // Stäng förslagslistan vid klick utanför
    ...
    handleContractNameInput(text) {
        ...
        // Föreslå verksamhetskod baserat på text
        if (results.verksamhet.length > 0) {
            const suggestedVerksamhet = results.verksamhet[0];
            this.suggestVerksamhetCode(suggestedVerksamhet);
        }
    }
    ...
    showSuggestions(results) {
        ...
        results.previousContracts.slice(0, 5).forEach(contract => {
            const li = document.createElement('li');
            li.innerHTML = `
                <div class="suggestion-text">${contract.text}</div>
                <div class="suggestion-details">
                    <span>Verksamhet: ${contract.verksamhet}</span>
                    <span>Ansvar: ${contract.ansvar}</span>
                    <span>Motpart: ${contract.motpart}</span>
                </div>
            `;
            li.addEventListener('click', () => this.applySuggestion(contract));
            suggestionsList.appendChild(li);
        });

        suggestionsList.style.display = results.previousContracts.length ? 'block' : 'none';
    }

    // Applicera valt förslag
    applySuggestion(contract) {
        document.getElementById('contractName').value = contract.text;
        document.getElementById('costVerksamhet').value = contract.verksamhet;
        document.getElementById('costAnsvar').value = contract.ansvar;
        document.getElementById('motpart').value = contract.motpart;
        
        // Uppdatera relaterade fält
        this.handleAnsvarChange(contract.ansvar);
        this.handleVerksamhetChange(contract.verksamhet);
    ...
    handleAnsvarChange(ansvar) {
        const verksamhet = document.getElementById('costVerksamhet').value;
        
        // Kontrollera om kombinationen är ovanlig
        if (verksamhet && !this.analyzer.isCommonCombination(ansvar, verksamhet)) {
            this.showUncommonCombinationWarning(ansvar, verksamhet);
        }

        // Uppdatera förslag på verksamhetskoder
        this.updateVerksamhetOptions(ansvar);

        // Kontrollera motpart-kombination
        const motpart = document.getElementById('motpart').value;
        if (motpart && !this.analyzer.isCommonAnsvarMotpart(ansvar, motpart)) {
            this.showUncommonMotpartWarning(ansvar, motpart);
        }
    }

    // Visa varning för ovanlig kombination
    showUncommonCombinationWarning(ansvar, verksamhet) {
        const message = `
            Ovanlig kombination av ansvar (${ansvar}) och verksamhet (${verksamhet}).
            Denna kombination förekommer sällan i historiska data.
            Vill du fortsätta med denna kombination?
        `;
        this.showWarningDialog(message, () => {
            // Callback om användaren väljer att fortsätta
            document.getElementById('uncommonCombinationConfirmed').value = 'true';
        });
    }

    // Visa varning för ovanlig motpart
    showUncommonMotpartWarning(ansvar, motpart) {
        const message = `
            Ovanlig kombination av ansvar (${ansvar}) och motpart (${motpart}).
            Denna kombination förekommer sällan i historiska data.
            Vill du fortsätta med denna kombination?
        `;
        this.showWarningDialog(message);
    ...
    handleVerksamhetChange(verksamhet) {
        const ansvar = document.getElementById('costAnsvar').value;
        
        if (ansvar && !this.analyzer.isCommonCombination(ansvar, verksamhet)) {
            this.showUncommonCombinationWarning(ansvar, verksamhet);
        }

        // Uppdatera aktivitetsrelaterade fält baserat på verksamhetskod
        this.updateActivityFieldsBasedOnVerksamhet(verksamhet);
    }

    // Uppdatera aktivitetsfält baserat på verksamhetskod
    updateActivityFieldsBasedOnVerksamhet(verksamhet) {
        const commonActivities = this.analyzer.getCommonActivitiesForVerksamhet(verksamhet);
        
        ['8036', '8035', '8032'].forEach(activity => {
            const container = document.getElementById(`activity${activity}Container`);
            if (container) {
                container.style.display = commonActivities.includes(activity) ? 'block' : 'none';
            }
        });
    }

    // Hantera ändring av totalbelopp
    handleTotalAmountChange(amount) {
        const numAmount = parseFloat(amount);
        if (isNaN(numAmount)) return;

        this.amountHandler.initializeContract(numAmount);
        this.updateActivityAmounts();
        this.validateAmounts();
    }

    // Hantera ändring av årsbelopp
    handleYearlyAmountChange(activity, yearlyAmount) {
        const monthlyAmount = Math.round(parseFloat(yearlyAmount) / 12);
        if (isNaN(monthlyAmount)) return;

        document.getElementById(`amount${activity}`).value = monthlyAmount;
        this.amountHandler.setActivity(activity, -monthlyAmount);
        this.validateAmounts();
    }

    // Validera alla belopp
    validateAmounts() {
        const totalCost = parseFloat(document.getElementById('totalAmount').value) || 0;
        return this.amountHandler.validateBalance(totalCost);
    }

    // Visa varningsdialog
    showWarningDialog(message, confirmCallback = null) {
        ...
        dialog.innerHTML = `
            <div class="warning-content">
                ...
                <div class="warning-buttons">
                    <button type="button" class="btn-continue">Fortsätt</button>
